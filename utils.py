...
def test_batch(test_utter_name):
    test_batch = []
    path = config.test_path
    utters = np.load(os.path.join(path,test_utter_name))
    #utters = np.transpose(utters)
    test_batch.append(utters[:,:])
    test_batch = np.transpose(test_batch, axes=(2,0,1)) 
    return test_batch
...
def cossim(x,y, normalized=False):
